...
`pmmrw` imputer selects an observed donor using a smooth PMM donor-selection
law and uses that donor's observed value as the imputation. It stores both the
smooth PMM working score and the selected donor row. Its bandwidth uses the
current empirically selected `3/4` donor-width scaling.
